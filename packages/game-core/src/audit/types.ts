...
export interface AuditEventPayloadMap {
  ...
  PLAYER_JOINED: { playerName: string; joinOrder: number };
  GAME_STARTED: {
    config: GameConfig;
  ...
  PROFESSION_ASSIGNED: { profession: Profession };
  BASE_INCOME: { amount: number };
  LAST_IN_ORDER_BONUS: { order: number; bonus: number };
  RANDOM_EVENT: { roll: number; threshold: number; bonus: number | null };
  CARD_DRAWN: {
    cardId: string;
    cardName: string;
  ...
  TURN_ORDER: {
    order: { playerId: string; name: string; order: number }[];
  };
  PLANNING_CONFIRMED: {
    profession: Profession | null;
  ...
  PLANNING_TIMEOUT: { skippedPlayerIds: string[] };
  RESOLUTION_STARTED: {
  ...
  PROTECTION: { profession: Profession };
  TAX_CATEGORY_SET: { taxedCategory: BuildingCategory };
  TAX_APPLIED: {
    builderId: string;
    builderName: string;
    buildingType: BuildingType;
  ...
  ROUND_FINISHED: { winnerId: string | null };
  GAME_FINISHED: {
    winnerId: string;
  ...
  INVALID_ACTION: { action: string; reason: string };
}
...
export interface AuditEventInput<
  T extends AuditEventType = AuditEventType,
> {
  type: T;
  phase: GamePhase;
  step: string;
 ...
 */
export interface AuditGoldChangeInput<
  T extends AuditEventType = AuditEventType,
> {
  type: T;
  phase: GamePhase;
...
export interface AuditGoldChangeRecord
  extends AuditGoldChangeInput,
